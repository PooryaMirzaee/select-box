...
کاربر ──HTTPS(443)──► OpenResty (خودِ 1Panel)
                          │  reverse proxy
                          ▼
                   nginx پروژه  (پورت 8080 روی هاست)
              ┌───────────┼───────────┐
              ▼           ▼            ▼
...
پس nginx داخلی پروژه را روی پورت **8080** می‌گذاریم و دامنه را با **Reverse Proxy** خود
...
پورت **8080** را عمومی **باز نکن** — فقط داخل سرور استفاده می‌شود.
...
# مهم: پورت داخلی nginx پروژه را 8080 بگذار تا با OpenResty خود 1Panel تداخل نکند
HTTP_PORT=8080
...
حالا سرویس روی `127.0.0.1:8080` بالاست. با OpenResty خود 1Panel آن را روی دامنه + SSL منتشر می‌کنیم.

1. در 1Panel برو به **Websites (网站) → Create Website (创建网站)**.
2. نوع را **Reverse Proxy (反向代理)** انتخاب کن.
3. مقادیر:
   - **Domain (主域名):** `shop.example.com`
   - **Proxy address (代理地址):** `http://127.0.0.1:8080`
4. ذخیره کن.
...
| پورت ۸۰ اشغال است هنگام up | یعنی `HTTP_PORT` را روی ۸۰ گذاشتی؛ آن را در `.env` به `8080` تغییر بده و دوباره up کن |
