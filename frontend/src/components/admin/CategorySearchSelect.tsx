  ...
  return (
    <div ref={rootRef} className={cn("relative min-w-[10rem]", className)}>
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        className={cn(
          "input-theme flex w-full items-center gap-1 px-2 py-1.5 text-start text-xs",
      ...
      {open ? (
        <div className="absolute start-0 z-40 mt-1 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-xl border border-theme bg-card shadow-lg">
          <div className="flex items-center gap-2 border-b border-theme px-2 py-1.5">
            ...
            />
            {q ? (
              <button type="button" className="rounded p-0.5 text-muted hover:text-[var(--fg)]" onClick={() => setQ("")}>
          ...
          <ul className="max-h-64 overflow-y-auto py-1" role="listbox">
            {filtered.length === 0 ? (
              <li className="px-3 py-4 text-center text-xs text-muted">دسته‌ای پیدا نشد</li>
            ) : (
              filtered.map((o, idx) => {
          ...
          <p className="border-t border-theme px-3 py-1.5 text-[10px] text-muted">
